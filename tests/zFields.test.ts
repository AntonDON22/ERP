...
describe('Универсальные схемы валидации числовых полей', () => {
  ...
  describe('zId - поля идентификаторов', () => {
    ...
    it('должен отклонять нулевые и отрицательные значения', () => {
      expect(() => zId.parse(0)).toThrow("ID должно быть больше нуля");
      expect(() => zId.parse(-1)).toThrow("ID должно быть больше нуля");
    });

    it('должен отклонять дробные числа', () => {
      expect(() => zId.parse(1.5)).toThrow("ID должно быть целым числом");
      expect(() => zId.parse("1.5")).toThrow("ID должно быть целым числом");
    });

    it('должен отклонять слишком большие значения', () => {
      expect(() => zId.parse(2147483648)).toThrow("ID слишком большое");
    });
  });
  ...
  describe('Строковые схемы (legacy)', () => {
    it('zPriceString должен валидировать строки с числами', () => {
      expect(zPriceString.parse("100")).toBe("100");
      expect(zPriceString.parse("0")).toBe("0");
      expect(() => zPriceString.parse("abc")).toThrow("Введите корректное значение цены");
      expect(() => zPriceString.parse("-1")).toThrow("Введите корректное значение цены");
    });

    it('zQuantityString должен валидировать строки с количествами', () => {
      expect(zQuantityString.parse("1")).toBe("1");
      expect(zQuantityString.parse("100")).toBe("100");
      expect(() => zQuantityString.parse("0")).toThrow("Введите корректное количество");
      expect(() => zQuantityString.parse("abc")).toThrow("Введите корректное количество");
    });

    it('zWeightString должен принимать пустые значения', () => {
      expect(zWeightString.parse("")).toBe("");
      expect(zWeightString.parse("100")).toBe("100");
      expect(() => zWeightString.parse("abc")).toThrow("Введите корректное значение веса");
    });

    it('zDimensionString должен принимать пустые значения', () => {
      expect(zDimensionString.parse("")).toBe("");
      expect(zDimensionString.parse("50")).toBe("50");
      expect(() => zDimensionString.parse("abc")).toThrow("Введите корректное значение размера");
    });
  });
