...
router.get("/", inventoryCache, async (req, res) => {
  try {
    ...
    try {
      ...
      apiLogger.error("API validation failed for inventory", {
    ...
    apiLogger.error("Failed to get inventory", {
...
router.get("/availability", inventoryCache, async (req, res) => {
  try {
    ...
    try {
      ...
      apiLogger.error("API validation failed for inventory availability", {
        error: validationError instanceof Error ? validationError.message : String(validationError),
      });
    }
    ...
    apiLogger.error("Failed to get inventory availability", {
      warehouseId: req.query.warehouseId,
      error: error instanceof Error ? error.message : String(error),
    });
