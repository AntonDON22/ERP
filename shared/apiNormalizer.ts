...
export function normalizeInventoryItem(rawItem: RawInventoryItem): NormalizedInventoryItem {
  ...
  const quantity = typeof quantityValue === "string" ? parseFloat(quantityValue) : quantityValue;
  ...
  if (rawItem.reserved !== undefined || rawItem.reserved_quantity !== undefined) {
    ...
    normalized.reserved =
      Number(typeof reservedValue === "string" ? parseFloat(reservedValue) : reservedValue) || 0;
  }
  ...
  if (rawItem.available !== undefined || rawItem.available_quantity !== undefined) {
    ...
    normalized.available =
      Number(typeof availableValue === "string" ? parseFloat(availableValue) : availableValue) || 0;
  }
...
export function normalizeInventoryArray(rawItems: RawInventoryItem[]): NormalizedInventoryItem[] {
  return rawItems.map((item, index) => {
    try {
      ...
      throw new Error(
        `Ошибка нормализации элемента ${index}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  });
}
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    for (const field of forbiddenFields) {
      if (item.hasOwnProperty(field)) {
        throw new Error(
          `API валидация: найдено запрещенное поле '${field}' в элементе ${i}. API должен возвращать только унифицированные поля.`
        );
      }
    }
  }
