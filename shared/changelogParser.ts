...
export function parseChangelogFromReplit(replitContent: string): DayData[] {
  ...
  const changelogStart = lines.findIndex(
    (line) => line.includes("## История изменений") || line.includes("## Changelog")
  );
...
function determineUpdateType(content: string): "feature" | "fix" | "improvement" | "database" {
  ...
  if (
    lowerContent.includes("база данных") ||
    lowerContent.includes("database") ||
    lowerContent.includes("материализованные") ||
  ...
  if (
    lowerContent.includes("исправ") ||
    lowerContent.includes("fix") ||
    lowerContent.includes("решена") ||
    lowerContent.includes("устранен") ||
    lowerContent.includes("ошибк") ||
    lowerContent.includes("баг")
  ) {
    return "fix";
  }

  if (
    lowerContent.includes("создан") ||
    lowerContent.includes("добавлен") ||
    lowerContent.includes("реализован") ||
    lowerContent.includes("новый") ||
    lowerContent.includes("модуль") ||
    lowerContent.includes("функциональность")
  ) {
    return "feature";
  }
