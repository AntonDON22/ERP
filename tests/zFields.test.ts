...
describe('Универсальные схемы валидации числовых полей', () => {
  ...
  describe('Строковые схемы (legacy)', () => {
    it('zPriceString должен валидировать строки с числами', () => {
      expect(// Старая схема удалена).toBe("100");
      expect(// Старая схема удалена).toBe("0");
      expect(() => // Старая схема удалена).toThrow("Введите корректное значение цены");
      expect(() => // Старая схема удалена).toThrow("Введите корректное значение цены");
    });
    ...
    it('zQuantityString должен валидировать строки с количествами', () => {
      ...
      expect(// Старая схема удалена).toBe("100");
      expect(() => // Старая схема удалена).toThrow("Введите корректное количество");
      expect(() => // Старая схема удалена).toThrow("Введите корректное количество");
    });

    it('zWeightString должен принимать пустые значения', () => {
      expect(// Старая схема удалена).toBe("");
      expect(// Старая схема удалена).toBe("100");
      expect(() => // Старая схема удалена).toThrow("Введите корректное значение веса");
    });

    it('zDimensionString должен принимать пустые значения', () => {
      expect(// Старая схема удалена).toBe("");
      expect(// Старая схема удалена).toBe("50");
      expect(() => // Старая схема удалена).toThrow("Введите корректное значение размера");
    });
  });
